...
class ProgramModel extends Model
{
    ...
    protected $allowedFields = [
        ...
        'organization_unit_id', 'description', 'description_en', 'credits',
    ...
    /**
     * Get programs by organization unit (หลักสูตรป.ตรี=4, บัณฑิต=5)
     */
    public function getByOrganizationUnit($organizationUnitId)
    {
        return $this->where('organization_unit_id', $organizationUnitId)
                    ->where('status', 'active')
                    ->orderBy('sort_order', 'ASC')
                    ->findAll();
    }
    ...
    /**
     * Get programs with organization unit info
     */
    public function getWithDepartment()
    {
        if (! $this->db->tableExists('organization_units') || ! $this->db->fieldExists('organization_unit_id', 'programs')) {
            return $this->where('programs.status', 'active')->orderBy('programs.sort_order', 'ASC')->findAll();
        }
        return $this->select('programs.*, organization_units.name_th as department_name, organization_units.name_en as department_name_en')
                    ->join('organization_units', 'organization_units.id = programs.organization_unit_id', 'left')
    ...
    /**
     * Get all programs (including inactive) with organization unit for admin
     */
    public function getAllWithDepartment()
    {
        if (! $this->db->tableExists('organization_units') || ! $this->db->fieldExists('organization_unit_id', 'programs')) {
            return $this->orderBy('programs.sort_order', 'ASC')->orderBy('programs.name_th', 'ASC')->findAll();
        }
        return $this->select('programs.*, organization_units.name_th as department_name, organization_units.name_en as department_name_en')
                    ->join('organization_units', 'organization_units.id = programs.organization_unit_id', 'left')
                    ->orderBy('programs.sort_order', 'ASC')
