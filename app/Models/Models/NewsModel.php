...
class NewsModel extends Model
{
    ...
    /**
     * Get published news by category (legacy: news.category) or by tag slug (news_tags)
     */
    public function getPublishedByCategory(string $category, int $limit = 10, int $offset = 0)
    {
        $db = \Config\Database::connect();
        if (!$db->fieldExists('category', 'news') && $db->tableExists('news_tags') && $db->tableExists('news_news_tags')) {
            return $this->getPublishedByTag($category, $limit, $offset);
        }
        if ($db->fieldExists('category', 'news')) {
            return $this->where('status', 'published')
